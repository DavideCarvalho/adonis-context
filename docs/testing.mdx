...
Code that reads `Context.userRef()` or `Context.tenantId()` needs an active context to read *from*. In production that context comes from the HTTP middleware (or a `Context.run` in a worker / command); in a unit test there is no request, so you build a fake one. The `@agora/context/testing` subpath gives you two helpers to do exactly that, with sensible defaults so you set only the fields your test cares about.

---

## Install

The helpers ship inside the core package as the `@agora/context/testing` subpath — there is nothing extra to install:

```sh
npm i @agora/context
```

The subpath re-uses the core `Context` under the hood, so the fake store it builds is the very same ALS store your production code reads from — there is no mock, just a real store with values you chose.
